...
const ProductCard = memo(
  ({ name, price, sale, image }) => {
    return (
      <div className="group relative flex flex-col gap-2 rounded-t-2xl hover:border-main hover:border-[1px] transition-all duration-100">
        ...
        <div className="relative rounded-2xl aspect-square">
          <Badge
            className={`${
              sale === 0 && "hidden"
            } absolute top-2 left-2 py-2 text-main`}
            variant={"outline"}
          >
            {`Sale ${sale} %`}
          </Badge>
